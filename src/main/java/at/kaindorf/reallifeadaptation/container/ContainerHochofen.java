package at.kaindorf.reallifeadaptation.container;
...
public class ContainerHochofen extends Container {
    private final TileEntityHochofen tileentity;
    private int cookTime, totalCookTime, burnTime, currentBurnTime;

    public ContainerHochofen(InventoryPlayer player, TileEntityHochofen tileentity) {
        this.tileentity = tileentity;
        IItemHandler handler = tileentity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null);
        this.addSlotToContainer(new SlotItemHandler(handler, 0, 56, 53));
        this.addSlotToContainer(new SlotItemHandler(handler, 1, 56, 15));
    ...
    public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (index == 3) {
                ...
                if (!CustormFurnaceRecipes.getInstance().getSinteringResult(stack1, slot1.getStack()).isEmpty()) {
                    if (!this.mergeItemStack(stack1, 0, 2, false)) {
                        return ItemStack.EMPTY;
                    } else if (TileEntityHochofen.isItemFuel(stack1)) {
